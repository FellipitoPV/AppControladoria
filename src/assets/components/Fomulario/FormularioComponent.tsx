...
const FormularioComponent: React.FC<FormularioComponentProps> = ({
  ...
  const [showDatePicker, setShowDatePicker] = React.useState<string | null>(
    null,
  );
  const [showSelectionModal, setShowSelectionModal] = React.useState<
    string | null
  >(null);
  const [showTimePicker, setShowTimePicker] = React.useState<string | null>(
    null,
  );
            ...
            <TextInput
              ...
              value={
                value instanceof dayjs
                  ? `${(value as Dayjs).format('DD/MM/YYYY')} às ${(
                      value as Dayjs
                    ).format('HH:mm')}`
                  : ''
              }
      ...
      case 'photos':
        return (
          <PhotoGallery
            ...
            onAddPhoto={newPhoto => onChange(field.name, [...photos, newPhoto])}
          />
        );
